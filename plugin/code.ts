...
type GradientStopSpec = { position: number; color: Rgba };
type BackgroundSpec =
  | { type: 'linear-gradient'; angle: number; stops: GradientStopSpec[] }
  | { type: 'radial-gradient'; stops: GradientStopSpec[] };
type MetadataSpec = {
  tag?: string;
  id?: string;
  classes?: string[];
...
type FrameSpec = BaseNodeSpec & {
  kind: 'frame';
  background?: Rgba | null;
  backgroundImageBytes?: Uint8Array | number[] | null;
  backgroundImageFit?: 'FILL' | 'FIT';
  gradient?: BackgroundSpec | null;
  borderColor?: Rgba | null;
  borderWidth?: number;
  radius?: [number, number, number, number];
  clipsContent?: boolean;
  shadow?: ShadowSpec | null;
  children: ImportNodeSpec[];
};

type TextSpec = BaseNodeSpec & {
  kind: 'text';
  text: string;
  color: Rgba;
  fontFamily: string;
  ...
  resizeMode: 'AUTO_WIDTH' | 'AUTO_HEIGHT';
  lineCount?: number;
  lockedLines?: boolean;
  originalText?: string;
};

type ImageSpec = BaseNodeSpec & {
  kind: 'image';
...
type SvgSpec = BaseNodeSpec & {
  kind: 'svg';
  markup: string;
};

type ImportNodeSpec = FrameSpec | TextSpec | ImageSpec | SvgSpec;
...
type EditableImportMessage = {
  type: 'import-editable';
  sourceName: string;
  width: number;
  height: number;
  rootStyle: { background?: Rgba | null };
  children: ImportNodeSpec[];
  batch?: BatchInfo;
  uiWarnings?: string[];
};
...
type PluginMessage = EditableImportMessage | ScreenshotImportMessage | { type: 'cancel' };

type FontRecord = { fontName: FontName; weight: number; italic: boolean };
type FontChoice = { fontName: FontName; matchedRequestedFamily: boolean; requestedPrimary: string };
type ProgressState = { done: number; total: number; warnings: Set<string> };
type BatchState = { nextX: number; baseY: number; roots: FrameNode[] };

const loadedFonts = new Set<string>();
let fontIndexPromise: Promise<Map<string, FontRecord[]>> | null = null;
...
function toFigmaColor(color: Rgba): RGB {
  return {
...
function rgbaToPaint(color: Rgba): SolidPaint {
  return {
    type: 'SOLID',
...
function styleWeight(style: string): number {
  ...
  if (s.includes('semi bold') || s.includes('semibold') || s.includes('demi bold')) return 600;
...
async function getFontIndex(): Promise<Map<string, FontRecord[]>> {
  if (!fontIndexPromise) {
    fontIndexPromise = figma.listAvailableFontsAsync().then((fonts) => {
      const index = new Map<string, FontRecord[]>();
...
async function chooseFont(requestedFamily: string, requestedWeight: number, italic: boolean): Promise<FontChoice> {
  ...
  const expanded: Array<{ family: string; requested: boolean }> = [];
  for (const family of requested) {
    const generic = genericFallbacks[family.toLowerCase()];
    if (generic) expanded.push(...generic.map((item) => ({ family: item, requested: false })));
    else expanded.push({ family, requested: true });
  }
  expanded.push(
    { family: 'Inter', requested: false },
    { family: 'Arial', requested: false },
    { family: 'Roboto', requested: false },
  );

  let records: FontRecord[] | undefined;
...
function applyCornerRadii(node: RectangleNode | FrameNode, radius?: [number, number, number, number]): void {
  if (!radius) return;
...
function applyShadow(node: FrameNode, shadow?: ShadowSpec | null): void {
  if (!shadow || shadow.color.a <= 0) return;
...
function gradientTransformFromCssAngle(angle: number): Transform {
...
function gradientToPaint(gradient: BackgroundSpec): GradientPaint {
...
function setMetadata(node: BaseNode, metadata?: MetadataSpec): void {
  if (!metadata) return;
  if (metadata.tag) node.setPluginData('htmlTag', metadata.tag);
  if (metadata.id) node.setPluginData('htmlId', metadata.id);
  if (metadata.classes?.length) node.setPluginData('htmlClasses', metadata.classes.join(' '));
  if (metadata.role) node.setPluginData('htmlRole', metadata.role);
  if (metadata.source) node.setPluginData('htmlSource', metadata.source);
}

function compactLayerName(value: string, fallback: string): string {
...
async function makeFrame(
  ...
  frame.opacity = clamp(spec.opacity ?? 1, 0, 1);
  ...
  if (spec.backgroundImageBytes) {
    try {
      ...
      fills.push({ type: 'IMAGE', imageHash: image.hash, scaleMode: spec.backgroundImageFit ?? 'FILL' });
    } catch (_error) {
      progress.warnings.add('Some CSS background images could not be created.');
    }
  }
  if (spec.gradient?.stops?.length) fills.push(gradientToPaint(spec.gradient));
  if (spec.background && spec.background.a > 0) fills.push(rgbaToPaint(spec.background));
  frame.fills = fills;

  if (spec.borderColor && (spec.borderWidth ?? 0) > 0) {
    frame.strokes = [rgbaToPaint(spec.borderColor)];
    frame.strokeWeight = Math.max(0, spec.borderWidth ?? 0);
  ...
  for (const child of spec.children ?? []) await createNode(child, frame, spec.x, spec.y, progress);
  return frame;
}
...
async function makeText(
  ...
  text.opacity = clamp(spec.opacity ?? 1, 0, 1);
  ...
  if (spec.originalText && spec.originalText !== spec.text) text.setPluginData('htmlOriginalText', spec.originalText);
  ...
  if (spec.resizeMode === 'AUTO_WIDTH') {
    ...
    if (spec.align === 'CENTER') text.x = targetX + (spec.width - text.width) / 2;
    else if (spec.align === 'RIGHT') text.x = targetX + spec.width - text.width;
    else text.x = targetX;
    ...
    if ((spec.lineCount ?? 1) === 1 && spec.height > 0) {
...
async function makeImage(
  ...
  rect.opacity = clamp(spec.opacity ?? 1, 0, 1);
...
async function makeSvg(
  ...
  try {
    ...
    if ('resize' in svg) svg.resize(safeDimension(spec.width), safeDimension(spec.height));
    svg.opacity = clamp(spec.opacity ?? 1, 0, 1);
...
async function createNode(
  ...
  let node: SceneNode;
  if (spec.kind === 'frame') node = await makeFrame(spec, parent, parentAbsX, parentAbsY, progress);
  else if (spec.kind === 'text') node = await makeText(spec, parent, parentAbsX, parentAbsY, progress);
  else if (spec.kind === 'image') node = await makeImage(spec, parent, parentAbsX, parentAbsY, progress);
  else node = await makeSvg(spec, parent, parentAbsX, parentAbsY, progress);
...
function countNodes(nodes: ImportNodeSpec[]): number {
  let count = 0;
  for (const node of nodes) {
    count += 1;
    if (node.kind === 'frame') count += countNodes(node.children ?? []);
  }
  return count;
}

function placeRoot(root: FrameNode, batch?: BatchInfo): BatchState | null {
  if (!batch) {
...
function finishRoot(root: FrameNode, batch?: BatchInfo): void {
  if (!batch) {
...
async function importEditable(message: EditableImportMessage): Promise<void> {
  const root = figma.createFrame();
  const preset = message.batch?.presetLabel ? ` — ${message.batch.presetLabel}` : '';
  ...
  if (message.batch?.presetLabel) root.setPluginData('viewportPreset', message.batch.presetLabel);
  placeRoot(root, message.batch);

  const progress: ProgressState = {
    done: 0,
    total: countNodes(message.children),
    warnings: new Set<string>(message.uiWarnings || []),
  };
  figma.ui.postMessage({ type: 'progress', done: 0, total: progress.total });
  for (const child of message.children) await createNode(child, root, 0, 0, progress);
...
async function importScreenshot(message: ScreenshotImportMessage): Promise<void> {
  const frame = figma.createFrame();
  const preset = message.batch?.presetLabel ? ` — ${message.batch.presetLabel}` : '';
...
figma.ui.onmessage = async (message: PluginMessage) => {
  ...
  try {
    if (message.type === 'import-editable') await importEditable(message);
    else if (message.type === 'import-screenshot') await importScreenshot(message);
  } catch (error) {
